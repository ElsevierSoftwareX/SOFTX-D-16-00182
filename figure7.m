...
for m=0:M:N-1
  mi=mi+1;
   ...
   XREZ=[XREZ, x]; 
     ...
     x=x+0*exp(-j*2*pi*tp*M*round(32*sin(m/N/2*2*pi-pi/5)));
...
S_x=[];
for i=0:N/M-1
S_x=[S_x; STFTFULLx(i*M+1:(i+1)*M)'];
end

Xorig=inv(AFULL)*STFTvec';
xorig=ifft(Xorig);
...
subplot(142),imagesc((abs(STFTsort)).^1),
set(gca,'xtick',[],'ytick',[])
xlabel({'Frequency samples','(b)'})
ylabel('Time samples')
text(N,N-64,' ','HorizontalAlignment','left','VerticalAlignment','bottom')

subplot(143),imagesc((abs(STFTfilt).^1)),
set(gca,'xtick',[],'ytick',[])
xlabel({'Frequency samples','(c)'})
ylabel('Time samples')
text(N,N-64,' ','HorizontalAlignment','left','VerticalAlignment','bottom')

subplot(1,4,4),imagesc((abs(STFTrecovered).^1)), axis xy
set(gca,'xtick',[],'ytick',[]), 
